...
Meteor.methods({
  ...
  lectureInsert: function(){
    ...
    var lectureId = Lectures.insert({'lectureCode': lectureCode, lectureCreator: userId});
    return {
      _id: lectureId,
  ...
  isLectureCreator: function(lectureCode) {
      
    result = Lectures.findOne({
        lectureCreator: Meteor.userId(),
        lectureCode: lectureCode,
      });  
