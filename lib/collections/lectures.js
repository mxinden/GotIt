...
Meteor.methods({
  insertLecture: function() {
    var lectureCode, lectureId;

    lectureCode = generateUniqueLectureCode();
    lectureId = App.Lectures.Collection.insert({
  ...
  setTitleOfLecture: function(lectureCode, title) {
    ...
    if (title.trim().length === 0) {
      throw new Meteor.Error('Empty Lecture title not allowed');
    }
...
App.Lectures.isLecturer = function(lectureCode) {
  var lecture;
  check(lectureCode, String);
  lecture = App.Lectures.Collection.findOne({lectureCode: lectureCode}, {fields: {lecturer: true}});
...
App.Lectures.getNumberOfStudents = function(lectureCode) {
  var lecture;
  check(lectureCode, String);
  lecture = App.Lectures.Collection.findOne({lectureCode: lectureCode}, {fields: {students: true}});
  if (lecture) {
