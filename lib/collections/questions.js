...
Meteor.methods({
  ...
  questionInsertAddVote: function(questionAttributes){
    ...
    var lecture = Lectures.findOne({lectureCode: questionAttributes.lectureCode});
  ...
  deleteQuestion: function(questionDeleteParams){
    ...
    if(isAuthor(lectureCode)){
      result = Questions.remove(questionId);
      if(result === false){
        throw new Meteor.Error('Could not delete question');
      }
      else{
        return true;
      }
