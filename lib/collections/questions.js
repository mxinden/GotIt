...
Meteor.methods({
  ...
  questionInsertAddVote: function(questionAttributes){
    ...
    Meteor.call('isLectureCreator', questionAttributes.lectureCode ,  function(error, result){
  ...
  deleteQuestion: function(questionDeleteParams){
    ...
    lectureCode = questionDeleteParams.lectureCode;
    questionId = questionDeleteParams.questionId;

    Meteor.call('isLectureCreator', lectureCode ,  function(error, result){
      /** Display error */
      if(error){
        return alert(error.reason);
      }
